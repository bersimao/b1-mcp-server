...
- In pinned mode the approved fingerprint **replaces** CA, hostname and validity checking rather than adding to it. After explicit origin-bound approval, the pin names one exact certificate and only its private-key holder can present it, so name matching — which exists to constrain a CA that signs many hosts — decides nothing further. Repeating it also made the mode unusable in practice: SAP B1 ships a self-signed Service Layer certificate issued to the machine's short name, while clients connect by FQDN, so an operator-approved certificate was rejected permanently with no route out through the interface. The trade accepted here is that a pin recorded against the wrong origin is no longer caught a second time by a name mismatch; the approval screen, which shows the certificate's subject, issuer, validity and the exact TLS error including the certificate's altnames, is the control that must catch it. Service Layer URLs may use DNS names or IP literals; `slTlsServerName`, when needed, is only a DNS SNI routing hint and not a certificate identity assertion.
