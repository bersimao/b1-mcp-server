...
8. **A default install carries vulnerable transitive packages.** `sps-sap-interface` pins its dependencies exactly (`axios: 0.25.0`, `express: 4.17.1`, `@sap/hana-client: 2.18.22`), so `npm audit` reports roughly 16 advisories, 9 high, in a consumer tree. The high-severity ones sit in `axios`, `express` and `cors`, which are reachable only through that package's `ServiceLayer.js` / `Xsjs.js` — neither of which this server imports. Service Layer traffic uses the local strict-TLS adapter and only `DirectDb` is loaded from the dependency, so the vulnerable code is installed but never executed here. This is a reachability argument, not a fix: the packages are still on disk, and a future import would activate them. `overrides` inside a published package are ignored by npm, so the root-level override block documented in [README.md](README.md#transitive-dependency-advisories) is the only consumer-side remedy (verified to reach `0 vulnerabilities`); it cannot apply under `npx`, which runs without a root `package.json`. The durable fix is upstream — loosened pins or dropping the unused HTTP dependencies from `sps-sap-interface`.
